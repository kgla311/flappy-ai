...
void Bird::Draw(SDL_Renderer * renderer, SDL_Texture* texture) {
    if (dead) {
        //SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255);  // Set color RED if dead, for debug
        return;
    }

    SDL_Rect dst = { static_cast<int>(x), static_cast<int>(y), BIRD_WIDTH, BIRD_HEIGHT };
